...
#' @param dv   Variable representing your dependent variable. Use the bare
#'   variable name when using \code{serr}.
#' @param id   Variable representing an id variable (e.g., "subject" or "item").
#'   Use the bare variable name when using \code{serr}.
#' @param ...  Variables to group by (e.g., conditions) when using \code{serr}.
#'   Use bare variable names spearated by commas.
#' @param confint_serr Size (in standard errors) for confidence intervals.
#'   Default confidence intervals are 1 SERR. To get 95% confidence intervals, enter 1.96.
